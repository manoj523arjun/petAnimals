...
export const CategoryBlock = (props) => {
	...
	if(categoryData.length>0) {
		if(type==="parent") {
			content = categoryData.map((data, id)=> <div key={data.cat_id} className="col-4 xs-block block-clear">
				<Link to={{pathname: 'category/'+data.breed_name.split(" ").join("_"), state: { data }}} className="content-block transition">
					<div className="block-overflow">
						<img className="transition" alt={data.breed_name} src={data.image} width="100%" />
					</div>
			...
			content = categoryData.map((data, id)=> <div key={id.toString()} className="col-4 xs-block block-clear">
